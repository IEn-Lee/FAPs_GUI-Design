...
void create_monitor_chart(lv_obj_t * parent, lv_coord_t x, lv_coord_t y) {
  ...
  lv_obj_set_style_bg_color(chart_temp_label, lv_palette_lighten(LV_PALETTE_GREY, 3), 0);  // 背景顏色
  lv_obj_set_style_bg_opa(chart_temp_label, LV_OPA_COVER, 0);  // 啟用背景填色
  lv_obj_set_style_pad_all(chart_temp_label, 4, 0);  // 內邊距讓文字不貼邊
  lv_obj_set_style_radius(chart_temp_label, 6, 0);   // 圓角
...
void lv_example_tabview_1(void) {
  ...
  create_status_cell(tab2, &humidity, "Humidity", "--%", lv_palette_main(LV_PALETTE_BLUE), 400, 200);
...
void loop() {
  ...
  if (millis() - last_update >= 1000) {
    ...
    if (temperature_value_label) {
      char temp_buf[16];
      snprintf(temp_buf, sizeof(temp_buf), "%.2f°C", temp);
      lv_label_set_text(temperature_value_label, temp_buf);
    }
