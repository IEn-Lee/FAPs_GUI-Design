...
void loop() {
  ...
  if (is_running) {
    ...
    int interval_between_cycles = 86400 / cycles_per_day; // interval of cycle
    ...
    if (!motor_is_running && (current_seconds - last_cycle_time >= interval_between_cycles)) {
      ...
      last_cycle_time = current_seconds; // update starting time
    }

    if (motor_is_running && (current_seconds - last_cycle_time >= cycle_duration)) {
      // finish this cycle, and stop motor
      stopMotor();
